...
// 
	public List<Student> savestudentANdSubjectDao(List<Student> stuList, List<Subject> sublisList) {
		try {
			et.begin();
	...
	public boolean insertstudentList(Student student) {
		List<Subject> suList = getAllSubject();

		try {
			et.begin();
			student.setSubList(suList);
			emEntityManager.persist(student);
			et.commit();
			return true;
		} catch (Exception e) {
	...
	public List<Subject> getAllSubject() {
		try {
			String selectQry = "From Subject";
			Query query = emEntityManager.createQuery(selectQry);
